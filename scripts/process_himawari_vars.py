...
fsspec_caching = {
    ...
    "block_size": 100 * 1024 * 1024 # size in bytes per block, adjust depends on the file size but the recommended size is in the MB}
}
...
def reprocess_himawari(ds, time, chunksize, **encoding_kwargs):
    def _prepare_aux_var(var: xr.DataArray, dtype) -> xr.DataArray:
        ...
        drop_coord_names = [
            name for name in ("latitude", "longitude", "Latitude", "Longitude") if name in da.coords
        ]
        if drop_coord_names:
        ...
        squeeze_dims = [dim for dim in da.dims if dim not in ("y", "x") and da.sizes.get(dim, 0) == 1]
        if squeeze_dims:
...
def get_himawari_patch(
    lat: float, lon: float, dataset: xr.Dataset, patch_size: int, sat_height: float = None, sat_lon_0: float = None, sat_lat_0: float = None
) -> xr.Dataset:
    """
    Get a patch of HIMAWARI data centered around a given latitude and longitude.
    """
    x, y = get_ahi_x_y(np.array([lat]), np.array([lon]), sat_height=sat_height, sat_lon_0=sat_lon_0, sat_lat_0=sat_lat_0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--HIMAWARI-file", type=str, required=True, help="Path to the file containing all the files to process")
    parser.add_argument("--save_path", type=str, required=True, help="Path to save the processed HIMAWARI patches")        
    ...
    if row['all_available']:
        ...
        sat_height = ast.literal_eval(ds.B01.orbital_parameters).get('projection_altitude')
        sat_lon_0 = ast.literal_eval(ds.B01.orbital_parameters).get('projection_longitude')
        sat_lat_0 = ast.literal_eval(ds.B01.orbital_parameters).get('projection_latitude')

        logger.info(f"Extracting patch ...")
        ds_patch = get_himawari_patch(lat, lon, ds, args.patch_size, sat_height=sat_height, sat_lon_0=sat_lon_0, sat_lat_0=sat_lat_0)
        ...
        ds_var_patch = get_himawari_patch(lat, lon, ds_var, args.patch_size, sat_height=sat_height, sat_lon_0=sat_lon_0, sat_lat_0=sat_lat_0)
