...
def download_mcmip(dt, patch_size, fov_radius):
    ...
    ds = xr.open_dataset(
        fs.open(abi_files["file"][0], **fsspec_caching), engine="h5netcdf"
    )
    ...
    crop = CenterWeightedCropDatasetEditor(
        patch_shape=(patch_size, patch_size), fov_radius=fov_radius, satellite="goes"
    )
    result = crop(ds)

    logger.info(
        f"Cropping dataset to patch of size {patch_size} with FOV radius {fov_radius} ..."
    )
...
if __name__ == "__main__":
    ...
    result = download_mcmip(dt, patch_size=args.patch_size, fov_radius=args.fov_radius)
    ...
    if result is not None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load saved patch file: {patch_filename}. Error: {e}"
            )
        ...
        logger.warning(
            f"No valid patch found for number {args.seed} and {dt}. Skipping upload ..."
        )
