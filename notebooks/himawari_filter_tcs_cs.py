...
def get_available_himawari_times(
    ...
    ahi_df = _himawari_file_df(
        "noaa-himawari8", "FLDK", start=start, end=end, ignore_missing=True
    )
    ...
    times = ahi_df.file.apply(
        lambda x: x.split("/")[5][:2] + ":" + x.split("/")[5][2:4] + ":00"
    )

    ahi_df["time"] = pd.to_datetime(
        years + "-" + months + "-" + days + " " + times, format="%Y-%m-%d %H:%M:%S"
    )
    ...
    for time, group in ahi_df.groupby("time"):
        if (
            len(group["file"].tolist()) >= 16
        ):  # Ensure at least 16 files for a valid time
            files.append(
                {
                    "start": time,
...
ibtracs_wp_file = (
    "/home/users/annaju/eo-data-prep/notebooks/IBTrACs/ibtracs.WP.list.v04r01.csv"
)
ibtracs_sp_file = (
    "/home/users/annaju/eo-data-prep/notebooks/IBTrACs/ibtracs.SP.list.v04r01.csv"
)
...
df_wp_filtered = df_wp_filtered[
    df_wp_filtered.LAT.between(-50, 50) & df_wp_filtered.LON.between(91, 191)
]
...
# filter for GOES FOV
df_sp_filtered = df_sp_filtered[
    df_sp_filtered.LAT.between(-50, 50) & df_sp_filtered.LON.between(91, 191)
]
# filter out storms with less than 8 points
...
df_all = df_all[
    (df_all["ISO_DATE"] >= datetime(2015, 7, 7).date())
    & (df_all["ISO_DATE"] <= datetime(2022, 12, 12).date())
]
df_all["ISO_TIME"] = _

SIDs = df_all.SID.unique()
print(f"Number of unique storms: {len(SIDs)}")
sum_df = pd.DataFrame(
    columns=[
        "start",
        "files",
...
for sid in tqdm(SIDs):
    ...
    longitudes, latitudes, timestamps = get_selected_tc(df_all, sid)
    ...
    ahi_df = get_available_himawari_times(
        pd.to_datetime(timestamps.min()), pd.to_datetime(timestamps.max())
    )
...
sum_df.to_csv("summary.himawari_ibtracs.SP-WP.list.v04r01.csv", index=False)
